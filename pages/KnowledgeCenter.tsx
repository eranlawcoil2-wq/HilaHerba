...
import { Search, Filter, ChevronRight, ChevronLeft, Sprout, FileText, BookOpen, X, Check } from 'lucide-react';
...
const KnowledgeCenter: React.FC = () => {
  ...
  const filteredContent = allContent.filter(item => {
    ...
    // 3. Tag Matching (If tags match any of the selected tags - OR logic within tags, AND logic with other filters)
    // Actually, usually users expect "AND" for tags? Or "OR"? Let's do "OR" (contains at least one of selected).
    let tagMatch = true;
      ...
      <div className="bg-[#1a2e1a] pt-32 pb-24 text-white rounded-b-[3rem] shadow-xl relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 text-center relative z-10">
          ...
          <p className="text-green-100 max-w-xl mx-auto text-lg font-light">
            מאגר המידע השלם: צמחי מרפא, מאמרים מקצועיים ומקרי בוחן.
          </p>
        </div>
      </div>
        ...
        {filteredContent.length === 0 && (
            <div className="text-center py-20 text-gray-400">
                ...
                <div className="flex gap-4 justify-center mt-4">
                    <button 
                        onClick={() => {setSearchTerm(''); setSelectedType('all'); setSelectedTags([])}}
